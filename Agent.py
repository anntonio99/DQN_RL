...
class Agent():
    ...
    def get_graph_features(self, environment, state_copy): # ---------------------------------------------------------------------------------------------------------------------
       ...
       # do not use the state from the environment, use state_copy
        ...
        bw_allocated_feature = state_copy[:,1]
        ...
        padding = self.hyperparamters['hidden_dim'] - environment.num_features # in num_features there is also bw_allocated, so we add 1
        ...
        features = tf.concat([tf.reshape(capacity_feature, (environment.num_edges, 1)),
                              ...
                              tf.reshape(bw_allocated_feature, (environment.num_edges, 1)),
                              padding
                              ], 
                              axis=1)
